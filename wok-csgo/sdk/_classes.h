...
public:
	...
	NETVAR_OFFSET(get_final_predicted_tick(), int, "CBasePlayer->m_nTickBase", sizeof(int))
		
	OFFSET(get_last_cmd(), c_user_cmd, 0x3298)
	OFFSET(is_defusing(), bool, 0x997C)
	...
	__forceinline vec3_t world_space_center( ) {
		if ( !this || !this->get_collideable( ) )
			return vec3_t( );

		vec3_t origin = this->get_abs_origin( );
		vec3_t min = origin + this->get_collideable( )->obb_mins( );
		vec3_t max = origin + this->get_collideable( )->obb_maxs( );
...
public:
	unsigned short type;
	unsigned short m_needs_to_interpolate; 
...
public:
	...
	OFFSET( is_jiggle_bones( ), bool, 0x2930 )
	OFFSET( should_use_new_animstate( ), bool, 0x9B14 )
	...
	VFUNC_SIG(setup_bones_attachment_helper( c_studio_hdr* hdr ), "client.dll", "55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4", void(__thiscall*)(void*, void*), hdr )

	player_info_t get_info();

	bool is_enemy(c_cs_player* from);
	bool is_friendly(c_cs_player* from);

	vec3_t get_hitbox_pos( int hitbox_id );
	vec3_t get_hitbox_pos( int hitbox_id, matrix3x4_t* best_matrix );
	...
	__forceinline void mark_as_interpolated( bool value ) {
		auto varmap = get< var_mapping_t >( 0x24 );
		for ( auto& item : varmap.m_entries )
			item.m_needs_to_interpolate = value;
	}
};
...
public:
	...
	DATA_MAP(get_next_burst_shot(), float, "m_fNextBurstShot")
	OFFSET(has_burst_mode(), bool, 13144)
	OFFSET(burst_shots_remain(), bool, 13284)
...
public:
	__forceinline c_base_combat_weapon* get( ) { return m_weapon; };
	__forceinline c_cs_weapon_data* data( ) { return m_data; };
	__forceinline int weapon_group( ) {
		/*int ret = weapon_general;
		if ( !m_data || !m_weapon )
			return ret;

		if ( m_data->m_weapon_type == WEAPON_TYPE_PISTOL )
			ret = weapon_pistols;

		if ( m_weapon->get_item_definition_index( ) == WEAPON_DESERT_EAGLE )
			ret = weapon_deagle;

		if ( m_weapon->get_item_definition_index( ) == 64 )
			ret = weapon_revolver;

		if ( m_data->m_weapon_type == WEAPON_TYPE_SHOTGUN || m_data->m_weapon_type == WEAPON_TYPE_MACHINE_GUN )
			ret = weapon_heavy;

		if ( m_data->m_weapon_type == WEAPON_TYPE_SMG )
			ret = weapon_pp;

		if ( m_data->m_weapon_type == WEAPON_TYPE_RIFLE )
			ret = weapon_rifles;

		if ( m_weapon->get_item_definition_index( ) == WEAPON_G3SG1 || m_weapon->get_item_definition_index( ) == WEAPON_SCAR20 )
			ret = weapon_auto_sniper;

		if ( m_weapon->get_item_definition_index( ) == WEAPON_SSG_08 )
			ret = weapon_ssg;

		if ( m_weapon->get_item_definition_index( ) == WEAPON_AWP )
			ret = weapon_awp;
	...
	__forceinline void init( c_cs_player* local ) {
		m_weapon = local->get_active_weapon( );
		if ( !m_weapon )
			return;

		m_data = m_weapon->get_cs_weapon_data( );
	}

	bool can_fire( int extra_ticks = 0 );
	bool is_shooting( );
private:
	c_base_combat_weapon* m_weapon;
...
public:
	...
	__forceinline void apply( c_cs_player* player ) {
		player->get_abs_velocity( ) = player->get_velocity( ) = m_velocity;
		player->get_eye_angles( ) = m_eye_angles;
		player->get_flags( ) = m_flags;
		player->get_origin( ) = m_origin;
		player->set_abs_origin( m_origin );

		std::memcpy( player->get_anim_layers( ).data( ), m_layers.data( ), sizeof( anim_layers_t ) );
		std::memcpy( player->get_bone_cache( )->m_cached_bones, m_matrix.data( ),sizeof( bone_matrix_t ) );
	}

	__forceinline void restore( c_cs_player* player ) {
		player->get_velocity( ) = m_velocity;
		player->get_abs_velocity( ) = m_abs_velocity;
		player->get_eye_angles( ) = m_eye_angles;
		player->get_flags( ) = m_flags;
		player->get_eflags( ) = m_eflags;
		player->get_origin( ) = m_origin;
...
public:
	...
	struct {
		std::array< bone_matrix_t, 3 > m_bone_matrix {};
		std::array< anim_layers_t, 3 > m_anim_layers {};
	} m_anim_data;
